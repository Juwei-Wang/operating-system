...
struct MemSim {
  ...
  type1 current;
  long page_size_k = 0;
  long request_page_num = 0;

  long getsize(type1 a){
      long k = a.size();
      return k;
  }
  ...
  void free( long tag) {
      ...
      for(long k = 0; k < getsize(current); ++k){
          ...
          if(current[true_index][0] == tag){
              ...
              if(true_index == (getsize(current) - 1)){
                  //printf("free size() loop \n");
                  //check left

                  if(getsize(current) >= 2){
              ...
              if((true_index < (getsize(current) - 1)) && (true_index > 0)){
      ...
      if(found){
          ...
          if(current[current.size() - 1][0] > 0){
               ...
               if(!div_exac) {
                   ...
                   current.push_back(temp_v);
                   request_page_num = request_page_num + size_m;
               } else{
                   request_page_num = request_page_num + size_m;
                   temp_v.push_back(tag);
                   temp_v.push_back(size);
