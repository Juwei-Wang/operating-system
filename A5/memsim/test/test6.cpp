...
using namespace std;
typedef std::vector<std::string> vs_t;    // type to split string
// to store arrival time , time interval , task_order
...
std::string RoundRobin(std::vector<pair1 > processes, long long time_slices ){
    ...
    while (1) {
        ...
        //break if no job will be executed
        if (waiting_job == 0) {
            break;
        }

        //check if the processes is in cpu_free status
        if(ready_queue.size() == 0){
            //printf("node add \n");
            process_change = false;
        }

        //put the task into ready queue
        //时间到了，把任务放进去
        ...
        //如果没有任何任务，cpu空置
        //if there is no task in last second, add -,to outcome
        ...
        //ready_queue is not empty , start to execute the task
        //有任务 任务开始执行
        if(temp_change > 0){
        ...
        //check time slice coming, change the task
        ...
        //if there is no task and cpu is free change the time when next task coming
        queue_add = false;
        ...
        //if there is a task and cpu is  not free, check the time slices and the time next task coming
...
//function to sort the current task
...
*/
    ...
    // add time_slice into the parameter
    std::string outcome;
    if (Sche_al.compare("RR")==0||Sche_al.compare("rr")==0){
        time_slices = std::stoll(argv[3]);
